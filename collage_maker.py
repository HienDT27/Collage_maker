...
def make_collage(images, filename, width=2000, init_height=250, fin_width=1920, fin_height=1080):
    ...
    for coef, imgs_line in coefs_lines:
        if imgs_line:
            ...
            for img_path in imgs_line:
                ...
                x += img.size[0] + margin_size
            # y += int(init_height) + margin_size 
            y += int(init_height / coef) + margin_size 
    # collage_image.save(filename)
    # print(collage_image.size)
    # print(collage_image.size[0])
    # print(collage_image.size[1])
    ...
    # else: 
    #     crop_image = collage_image.thumbnail((int(width / coef), int(init_height / coef)), Image.ANTIALIAS)
        # baseheight = init_height
        # hpercent = (baseheight / float(img.size[1]))
        # wsize = int((float(img.size[0]) * float(hpercent)))
        # img = img.resize((wsize, baseheight), PIL.Image.ANTIALIAS)
        # crop_image = collage_image.resize((fin_width,fin_height))
    # collage_image.save(filename)
